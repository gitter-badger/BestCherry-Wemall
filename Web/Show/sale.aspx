            ...
            <li>
                ...
                <div class="d_title">
                    <span class="title">
                        <%=item.TITLE %>
                        <br />
                        ￥<label style="font-size:2rem;"><%=item.MEMBERPRICE %></label>
                    </span>
                </div>
                <img src="<%=item.IMAGEPATH %>" alt="" name="timebuy" code="<%=item.CODE %>" id="<%=item.ACTIVITYID %>" />
                <div class="d_buy">
                    <button type="button" name="buy" class=" buy am-radius am-btn am-btn-danger" procode="<%=item.CODE %>" id="<%=item.ACTIVITYID %>"><%=CheckTime(item.BUYTIME) %> </button>
                </div>
            </li>
            <%} %>
        </ul>
            ...
            <ul class="am-list">
                ...
                <li class="am-g am-list-item-desced am-list-item-thumbed am-list-item-thumb-left">
                    ...
                    <div class="am-u-sm-8 am-list-main">
                        <h3 class="am-list-item-hd" style="font-size: 15px; color: #dd514c; font-weight: bold;">
                            <%=item.TITLE %> ￥<%=item.PRICE %>
                        </h3>
                        <div class="am-list-item-text"><%=item.SMALLTITLE %></div>
                        <button class="am-btn am-radius am-topbar-btn am-btn-sm am-btn-danger" style="float: right" onclick="window.location.href=' introduce.aspx?procode=<%=item.CODE %>&type=2&activityid=<%=item.ACTIVITYID %>'">立即购买</button>
                    </div>
                </li>
                <%} %>
            </ul>
            ...
            <li>
                <div class="am-gallery-item">
                    <a>
                        ...
                        <h3><%=item.TITLE %>
                            <br />
                            当前：<%=item.POLL %>票<a name="poll" code="<%=item.PROID %>" style="float: right">投票</a></h3>
                    </a>
                </div>
            </li>
            <%} %>
