var type=$('.address-form input[name=type]').val();
if (type == "1") {
...
$("#submitOrder").click(function () {
    ...
    $("table[name=order-list]").find("tr").each(function () {
        ...
        $(this).find('input[name=address]').val(address);
        orderstr += $(this).attr("id") + "," + $(this).find("[name=buynum]").val() + ",";
    });
...
$("button[name=min]").click(function () {
    t = $(this).next();
    if (t.text() == "0") {
        return false;
    }
...
function setTotal() {
    ...
    $("[name=buynum]").each(function () {
        ...
        var price = $(this).nextAll("[name=oneprice]").val();
        if (isMN) {
            price -= 10;
        }
        var num = $(this).text();
    ...
    if (hasYunfei == true) {
        $(".price").html("总计：" + (allPrice).toFixed(2) + "(含运费10)");
    } else {
